 ...
 * Handles registering and unregistering the application with the YARN ResourceManager.
  * 用yarn ResourceManager,处理注册和移除注册应用
 */
private[spark] class YarnRMClient extends Logging {
  ...
  def register(
    ...
    synchronized {
      amClient.registerApplicationMaster(driverHost, driverPort, uiHistoryAddress)
      registered = true
    }
  }
  ...
  def getMaxRegAttempts(sparkConf: SparkConf, yarnConf: YarnConfiguration): Int = {
    val sparkMaxAttempts = Option.apply(2)//sparkConf.get(MAX_APP_ATTEMPTS).map(_.toInt)
