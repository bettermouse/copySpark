...
/**
  ...
  * 从 executor到driver一个心跳,这是一个共享消息被几个内部组件使用来报活/正在执行任务的执行信息
  * 它将过期主机(持续 spark.network.timeout 没有心跳)
  */
private[spark] case class Heartbeat(
                                     executorId: String) // executor level updates
...
private[spark] class HeartbeatReceiver(sc: SparkContext, clock: Clock)
  extends SparkListener with Actor with Logging {
  ...
  val system: ActorSystem = sc.env.actorSystem

  //private[spark] var scheduler: TaskScheduler = null

  // executor ID -> timestamp of when the last heartbeat from this executor was received
  // 最后的从这个executor的心跳被收到的时间戳
  private val executorLastSeen = new mutable.HashMap[String, Long]

  // "spark.network.timeout" uses "seconds", while `spark.storage.blockManagerSlaveTimeoutMs` uses
  // "milliseconds"
  private val executorTimeoutMs = 120000L
  //    sc.conf.getTimeAsMs("spark.storage.blockManagerSlaveTimeoutMs",
  //      s"${sc.conf.getTimeAsSeconds("spark.network.timeout", "120s")}s")

  // "spark.network.timeoutInterval" uses "seconds", while
  // "spark.storage.blockManagerTimeoutIntervalMs" uses "milliseconds"
  private val timeoutIntervalMs = 60000L
  //    sc.conf.getTimeAsMs("spark.storage.blockManagerTimeoutIntervalMs", "60s")
  private val checkTimeoutIntervalMs = 60000L
  ...
  override def preStart(): Unit = {
    ...
    timeoutCheckingTask = eventLoopThread.scheduleAtFixedRate(new Runnable {
      override def run(): Unit = Utils.tryLogNonFatalError {
        //检查executor是否过期
        Option(self).foreach(_.!(ExpireDeadHosts))
      }
  ...
  override def receive: Receive = {
    ...
    case heartbeat@Heartbeat(executorId) =>
      //if (scheduler != null) {
      log.info("收到心跳"+executorId)
      if (true) {
  ...
  def addExecutor(executorId: String): Unit = {
    Option(self).map(_.!(ExecutorRegistered(executorId)))
  }
  ...
  def removeExecutor(executorId: String): Unit = {
    Option(self).map(_.!(ExecutorRemoved(executorId)))
  }
