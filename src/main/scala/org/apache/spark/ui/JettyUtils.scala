...
/**
  ...
  * 加载一个 用 jetty的http server 类 加载一个web server的工具
  */
private[spark] object JettyUtils extends Logging {
  ...
  // 基于Http请求返回一些东西的函数的基本类型
  // 允许从许多类型的函数隐式转换为jetty Handler。
  ...
  //功能描述：创建javax.servlet.http.HttpServlet的匿名内部类实例。
  // 此实例处理请求实际是使用servletParams的responder：Responder，
  // 此Responder类型发生隐式转换，会转换为用户传入的函数参数。
  ...
  /** Create a context handler that responds to a request with the given path prefix
    * 创建一个 响应一个给定路径的请求context handler */
  def createServletHandler[T <: AnyRef](
                                         path: String,
                                         servletParams: ServletParams[T],
                                         conf: SparkConf,
                                         basePath: String = ""): ServletContextHandler = {
    //1) 调用createServlet，生成javax.servlet.http.HttpServlet的匿名内部类实例。
    // 此实例处理请求实际是使用servletParams的responder：Responder，
    // 此Responder类型发生隐式转换，会转换为用户传入的函数参数。
  ...
  def startJettyServer(
    ...
    try {
      ...
      ServerInfo(server, httpPort, conf, collection)
    } catch {
      case e: Exception =>
        server.stop()
...
//提供给Jetty服务器添加或移除ContextHandler，以及停止Jetty服务器的实现
